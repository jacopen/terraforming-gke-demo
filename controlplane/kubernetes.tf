...
resource "null_resource" "setup_kubernetes" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sed -i -e 's/INGRESS_IP/${google_compute_address.cp_ingress_ip.address}/g' /tmp/manifests/nginx-ingress.yaml",
      "/snap/bin/kubectl apply -n argocd -f /tmp/manifests/argocd.yaml",
      "/snap/bin/kubectl apply -n argocd -f /tmp/manifests/secrets.yaml",
      "/snap/bin/kubectl apply -n argocd -f /tmp/manifests/argocd-config.yaml",
      "/snap/bin/kubectl apply -n argocd -f /tmp/manifests/apps.yaml",
      "/snap/bin/kubectl apply -f /tmp/manifests/nginx-ingress.yaml",
      "/snap/bin/kubectl apply -n argocd -f /tmp/manifests/ingress.yaml",
    ]
  }
}